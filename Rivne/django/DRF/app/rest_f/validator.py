from rest_framework.utils.representation import smart_repr
from rest_framework import serializers
import re


class AddressValidator:
    message = "Validation error: "

    def __init__(self, message=None):
        self.validated_filed = ''
        self.validated_data = None
        self.message = message or self.message

    def __call__(self, attrs):
        message = self.message
        if 'apartaments' in attrs and attrs['apartaments'] <= 0:
            message = 'cannot be less than zero'
            self.raize_error(attrs, 'apartaments', message)
        elif 'country' in attrs and len(attrs['country']) <= 3:
            message = f'Can not be shorter than 3 characters'
            self.raize_error(attrs, 'country', message)
        elif 'city' in attrs and len(attrs['city']) <= 3:
            message = f'Can not be shorter than 3 characters'
            self.raize_error(attrs, 'city', message)
        elif 'house_num' in attrs and attrs['house_num'] <= 0:
            message = 'cannot be less than zero'
            self.raize_error(attrs, 'house_num', message)
        elif 'zip_code' in attrs and 9999 > attrs['zip_code']:
            message = 'Must be a five-digit number'
            self.raize_error(attrs, 'zip_code', message)
        elif 'street' in attrs:
            pattern = re.compile("(улица|проспект|бульвар|переулок)")
            match = pattern.search(attrs['street'])
            if not match:
                message = 'must contain: (улица|проспект|бульвар|переулок)'
                self.raize_error(attrs, 'street', message)

    def raize_error(self, attrs, field_name, message):
        message = self.message + f'The {field_name} field {message}.'
        self.validated_filed = field_name
        self.validated_data = attrs[field_name]
        raise serializers.ValidationError(message, code=field_name)

    def __repr__(self):
        return f'<{self.__class__.__name__}({self.validated_filed}={self.validated_data})>'