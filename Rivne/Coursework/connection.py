...
class Connection:
    ...
    def getNextId(self, table):
        table = (table,)
        fields = ('id',)
        if not self.getData(table, fields):
            return 1
        result = self.getData(table, fields)[-1][0] + 1
        return result

    def loginCheck(self, login, password, role):
        find_login = self.getData(('users',), ('*',), f" where login = '{login}'")
        if find_login and password == find_login[0][2] and find_login[0][5] == role:
